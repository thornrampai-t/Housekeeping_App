...
class FirestoreService {
  ...
  Stream<QuerySnapshot> getWhereDeepCleanStream() {
    final deepCleanStream =
        employee.where('serviceType', isEqualTo: 'แม่บ้าน').snapshots();
    return deepCleanStream;
  }

  Stream<QuerySnapshot> getWhereGardenStream() {
    final deepCleanStream =
        employee.where('serviceType', isEqualTo: 'คนสวน').snapshots();
    return deepCleanStream;
  }
  ...
  Stream<QuerySnapshot> getWherePetStream() {
    final deepCleanStream =
        employee.where('serviceType', isEqualTo: 'ดูแลสัตว์').snapshots();
    return deepCleanStream;
  }
    ...
    return employee
        ...
        .map((snapshot) {
          if (snapshot.docs.isNotEmpty) {
            return snapshot.docs.first['serviceType'] ?? 'ไม่พบข้อมูล';
          }
          return 'ไม่พบข้อมูล';
        });
  }

//   Future<String> getImageEmployee(String customerId) async {
//   try {
//     // 1. ดึงข้อมูลของพนักงานจาก Firestore
//     DocumentSnapshot docSnapshot = await FirebaseFirestore.instance
//         .collection('employees') // เปลี่ยนเป็น collection ที่คุณใช้จริง
//         .doc(customerId)
//         .get();

//     if (!docSnapshot.exists) {
//       throw Exception("ไม่พบข้อมูลพนักงานที่มี ID: $customerId");
//     }

//     // 2. ดึงค่าของ field 'imagePath' ที่เก็บ path ของรูป
//     String filePath = docSnapshot.get('imagePath'); 

//     if (filePath.isEmpty) {
//       throw Exception("ไม่พบ path ของรูปภาพสำหรับพนักงาน ID: $customerId");
//     }

//     print("พบ imagePath: $filePath");

//     // 3. ดึง URL ของรูปจาก Firebase Storage
//     String downloadUrl =
...
//     throw Exception("ไม่สามารถดึงรูปภาพได้");
//   }
// }
Future<String> getImageEmpolyee(String filePath) async {
  ...
  Stream<QuerySnapshot> getHistoryandBookmarkCustomerStream(String userId) {
  ...
  Future<void> addBookLocation(
    ...
    if (querySnapshot.docs.isNotEmpty) {
      ...
      await customerDoc.update({
        'addresses': FieldValue.arrayUnion([listMarkLocation]),
      });

      print("✅ เพิ่มข้อมูลการจองลงใน address สำเร็จ");
    } else {
      print("❌ ไม่พบลูกค้าในฐานข้อมูล");
    }
  }

  Future<void> updateFullNameCustomer(
